...
class TestSearchFiles(unittest.TestCase):
    def setUp(self):
        ...
        with open(os.path.join(self.tmp, 'a.py'), 'w') as f: f.write('def hello():\n    return 42\n')
        with open(os.path.join(self.tmp, 'b.txt'), 'w') as f: f.write('hello world\nfoo bar\n')
    def test_search_literal(self):
        r = _search_files({'pattern': 'hello', 'path': self.tmp}, self.wg, self.rg)
        self.assertTrue(r.success); self.assertIn('hello', r.content)
    def test_search_icase(self):
        r = _search_files({'pattern': 'HELLO', 'path': self.tmp, 'ignore_case': True}, self.wg, self.rg)
        self.assertTrue(r.success); self.assertIn('hello', r.content.lower())
...
class TestPytestOutput(unittest.TestCase):
    def test_passing(self):
        ...
        self.assertIsInstance(s, str); self.assertIn('2 passed', s)
    def test_failing(self):
        o = 'tests/test.py::test FAILED [100%]\n===== 1 failed ====='
        s, f = _parse_pytest_output(o, exit_code=1)
        self.assertIsInstance(s, str); self.assertIn('1 failed', s)
