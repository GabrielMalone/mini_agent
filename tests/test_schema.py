...
class TestSchemaStructure(unittest.TestCase):
    ...
    def test_every_tool_has_type_function(self):
        for td in TOOLS:
            self.assertEqual(
                td.get("type"), "function", f"Tool missing type=function: {td}"
            )

    def test_every_tool_has_function_dict(self):
        for td in TOOLS:
            self.assertIsInstance(
                td.get("function"), dict, f"Tool missing function dict: {td}"
            )

    def test_every_tool_has_name(self):
        for td in TOOLS:
            name = td.get("function", {}).get("name")
            self.assertIsInstance(name, str, f"Tool missing function.name: {td}")
            self.assertGreater(len(name), 0, f"Tool has empty function.name: {td}")
...
class TestSchemaCoverage(unittest.TestCase):
    ...
    def test_all_core_tools_have_schemas(self):
        ...
        for name in CORE_TOOLS:
            self.assertIn(
                name, schema_names, f"CORE_TOOLS entry '{name}' has no schema in TOOLS"
            )
        ...
        for td in TOOLS:
            ...
            for param_name, param_def in props.items():
                ...
                self.assertIsNotNone(
                    ptype, f"Tool '{name}', param '{param_name}' has no type"
                )
                self.assertIn(
                    ptype,
                    self.VALID_TYPES,
