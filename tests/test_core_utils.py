...
class TestToolResult(unittest.TestCase):
    ...
    def test_validation_error(self):
        ...
        r = ToolResult.validation_error(
            "bad input", hint="use string", valid_params="name: str"
        )
        self.assertFalse(r.success)
...
class TestRepairToolCalls(unittest.TestCase):
    ...
    def test_flatten_keeps_normal_args(self):
        ...
        tcs = [
            {"function": {"name": "read_file", "arguments": '{"path": "/normal.py"}'}}
        ]
        result = _flatten_nested_params(tcs)
        import json
...
class TestErrorHints(unittest.TestCase):
    ...
    def test_build_hint_for_write_file_blocked(self):
        ...
        self.assertTrue(
            "safety" in hint.lower()
            or "outside" in hint.lower()
            or "workspace" in hint.lower()
        )
...
class TestImmutablePrefix(unittest.TestCase):
    ...
    def test_different_few_shots_different_fingerprint(self):
        ...
        p1 = ImmutablePrefix(
            system="sys", tool_specs=[], few_shots=[{"role": "user", "content": "a"}]
        )
        p2 = ImmutablePrefix(
            system="sys", tool_specs=[], few_shots=[{"role": "user", "content": "b"}]
        )
        self.assertNotEqual(p1.fingerprint, p2.fingerprint)
