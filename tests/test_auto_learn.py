    ...
    def test_all_values_are_str_to_str_dicts(self) -> None:
        ...
        for tool, fingerprints in _FAILURE_PATTERNS.items():
            self.assertIsInstance(
                fingerprints, dict, f"Entry for '{tool}' is not a dict"
            )
            for key, hint in fingerprints.items():
                self.assertIsInstance(key, str, f"Key in '{tool}' is not str: {key!r}")
                self.assertIsInstance(
                    hint, str, f"Hint for '{tool}:{key}' is not str: {hint!r}"
                )
...
class TestFingerprintError(unittest.TestCase):
    ...
    def test_edit_file_not_found(self) -> None:
        self.assertEqual(
            _fingerprint_error("edit_file", "old_string not found in file"), "not found"
        )
        self.assertEqual(
            _fingerprint_error("edit_file", "file does not exist"), "not found"
        )
    ...
    def test_edit_file_whitespace(self) -> None:
        ...
        self.assertEqual(
            _fingerprint_error("edit_file", "check indentation"), "whitespace"
        )
        self.assertEqual(
            _fingerprint_error("edit_file", "tab characters found"), "whitespace"
        )
        self.assertEqual(
            _fingerprint_error("edit_file", "trailing whitespace"), "whitespace"
        )

    def test_edit_file_ambiguous(self) -> None:
        self.assertEqual(
            _fingerprint_error("edit_file", "ambiguous match"), "ambiguous"
        )
        self.assertEqual(
            _fingerprint_error("edit_file", "multiple occurrences"), "ambiguous"
        )
        self.assertEqual(
            _fingerprint_error("edit_file", "old_string appears twice"), "ambiguous"
        )

    def test_edit_file_count(self) -> None:
        self.assertEqual(
            _fingerprint_error("edit_file", "invalid count value"), "count"
        )
        self.assertEqual(
            _fingerprint_error("edit_file", "count parameter error"), "count"
        )

    def test_write_file_blocked(self) -> None:
        self.assertEqual(
            _fingerprint_error("write_file", "write blocked by safety layer"), "blocked"
        )
        self.assertEqual(
            _fingerprint_error("write_file", "safety check failed"), "blocked"
        )

    def test_write_file_exists(self) -> None:
        self.assertEqual(
            _fingerprint_error("write_file", "file already exists"), "exists"
        )
        self.assertEqual(
            _fingerprint_error("write_file", "overwrite is disabled"), "exists"
        )

    def test_read_file_not_found(self) -> None:
        self.assertEqual(_fingerprint_error("read_file", "file not found"), "not found")
        self.assertEqual(
            _fingerprint_error("read_file", "no such file or directory"), "not found"
        )

    def test_read_file_offset(self) -> None:
        self.assertEqual(
            _fingerprint_error("read_file", "offset exceeds file length"), "offset"
        )
        self.assertEqual(
            _fingerprint_error("read_file", "offset 5000 exceeds"), "offset"
        )

    def test_search_files_not_found(self) -> None:
        self.assertEqual(
            _fingerprint_error("search_files", "no matches found"), "not found"
        )
        self.assertEqual(
            _fingerprint_error("search_files", "pattern not found"), "not found"
        )

    def test_search_files_invalid_regex(self) -> None:
        self.assertEqual(
            _fingerprint_error("search_files", "invalid regex pattern"), "invalid regex"
        )

    def test_run_shell_not_found(self) -> None:
        self.assertEqual(
            _fingerprint_error("run_shell", "command not found"), "not found"
        )
        self.assertEqual(
            _fingerprint_error("run_shell", "bash: thing: not found"), "not found"
        )

    def test_run_shell_blocked(self) -> None:
        self.assertEqual(
            _fingerprint_error("run_shell", "blocked by safety"), "blocked"
        )
        self.assertEqual(
            _fingerprint_error("run_shell", "destructive command blocked"), "blocked"
        )

    def test_run_shell_timed_out(self) -> None:
        self.assertEqual(
            _fingerprint_error("run_shell", "command timed out"), "timed out"
        )
        self.assertEqual(
            _fingerprint_error("run_shell", "timeout after 60 seconds"), "timed out"
        )

    def test_find_symbol_not_found(self) -> None:
        self.assertEqual(
            _fingerprint_error("find_symbol", "no matches for symbol"), "not found"
        )
        self.assertEqual(_fingerprint_error("find_symbol", "not found"), "not found")

    def test_find_usages_not_found(self) -> None:
        self.assertEqual(
            _fingerprint_error("find_usages", "no matches for usages"), "not found"
        )
    ...
    def test_verify_failures(self) -> None:
        self.assertEqual(
            _fingerprint_error("verify", "verification FAILED"), "failures"
        )
...
class TestLearnFromFailure(unittest.TestCase):
    ...
    def _make_result(
        self, success: bool = False, content: str = "", hint: str = ""
    ) -> ToolResult:
    ...
    def test_unclassified_pattern_generic_hint_on_third(self) -> None:
        ...
        self.assertEqual(
            result2.hint, "", "No hint on 2nd failure for unclassified pattern"
        )
    ...
    def test_different_fingerprints_count_separately(self) -> None:
        ...
        self.assertEqual(
            result_b1.hint, "", "Different fingerprint should start at count=1"
        )
